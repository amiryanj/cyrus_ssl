...
void SSLRobotKalmanFilter::runFilter()
{
    ...
    if( rawPositionList.size() >= 2 )
    {
        ...
        filteredPosition.x = x(1);
        filteredPosition.y = x(2);
        filteredPosition.teta = x(3);

        filteredSpeed.x = x(4);
        filteredSpeed.y = x(5);
        filteredSpeed.teta = x(6);
...
{
    for(int i=1; i <= W.nrow(); i++)
        for (int j=1; j<= W.ncol(); j++)
            W(i, j) = 0.0;
...
void SSLRobotKalmanFilter::makeQ()
{
    ...
    for(int i=1; i <= Q.nrow(); i++)
        for (int j=1; j<= Q.ncol(); j++)
        {
            if( i == j)
                Q(i, j) = q[i-1];
            else
